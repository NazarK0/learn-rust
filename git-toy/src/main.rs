...
fn check(activity: &'static str, status: c_int) -> c_int {
    if status < 0 {
        unsafe {
            ...
            println!("error while {activity}: {message} ({klass})",
                message = CStr::from_ptr(error.message).to_string_lossy(),
                klass = error.klass);
            std::process::exit(1);
        }
    }
...
unsafe fn show_commit(commit: *const raw::git_commit) {
    ...
    println!("{message}", message = CStr::from_ptr(message).to_string_lossy());
